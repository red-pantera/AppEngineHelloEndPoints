...
function enableButtons () {
    ...
    btn = document.getElementById("input_profession");
    btn.onclick=function(){greetByProf();};

    // Update the button label now that the button is active
    btn.value="Click me and I will know your profession";
...
function greetByProf () {
    // Get the name from the name_field element
    var prof = document.getElementById("profession_field").value;

    // Call the sayHelloByName() function.
    // It takes one argument "name"
    // On success, pass the response to sayHelloCallback()
    var request = gapi.client.helloworldendpoints.sayHelloByName({'name': prof});
    request.execute(sayHelloCallback);
}
